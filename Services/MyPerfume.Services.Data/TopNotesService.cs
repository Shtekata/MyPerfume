namespace MyPerfume.Services.Data
{
    ...
    {
        private readonly IDeletableEntityRepository<TopNote> deletableEntityRepository;

        public TopNotesService(IDeletableEntityRepository<TopNote> deletableEntityRepository)
        {
        ...
        {
            var model = new TopNote { Name = input.Name };
        ...
        {
            IQueryable<TopNote> query = this.deletableEntityRepository.AllAsNoTracking()
            .OrderBy(x => x.Name);
            if (count.HasValue)
            {
        ...
        public TopNote GetByIdModel(string id)
        {
