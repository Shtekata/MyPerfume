namespace MyPerfume.Services.Data
{
    ...
    public class PerfumersService : IPerfumersService
    {
        private readonly IDeletableEntityRepository<Perfumer> deletableEntityRepository;

        public PerfumersService(IDeletableEntityRepository<Perfumer> deletableEntityRepository)
        {
        ...
        {
            IQueryable<Perfumer> query = this.deletableEntityRepository.AllAsNoTracking()
            .OrderBy(x => x.Name);
            if (count.HasValue)
            {
        ...
        public Perfumer GetByIdModel(string id)
        {
